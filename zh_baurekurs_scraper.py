...
# python3.10 zh_baurekurs_scraper.py -p ZH_Baurekurs -s ZH_Baurekurs_clean
...
def split_lines(parsed_text: str) -> List[str]:
    split_lines = [line.strip().replace("     ", " ").replace("\uf02d", "").replace(" ", "").replace("", "").replace("_______________________________________________________", "") for line in parsed_text.split("\n")]
    return split_lines


def get_pages(lines: List[str]) -> str:
    page_pattern1 = r"\S+\.\d+\.\d+\sSeite\s\d+"
    if re.search(page_pattern1, " ".join(lines)):
        pages = [lines.pop(lines.index(line)).split()[-1] for line in lines if re.fullmatch(page_pattern1, line)]
    else:
...
def get_paras(lines: List[str]) -> List[str]:
    ...
    for i, line in enumerate(lines):
        ...
        if "G.-Nr." in " ".join(lines) and (line == "hat sich ergeben:" or line == "1.") and sachverhalt_counter == 0:
            clean_lines.append(line.strip())
            sachverhalt_counter += 1
            continue

        if "G.-Nr." in " ".join(lines) and sachverhalt_counter == 0 and line:
            clean_lines.append(line.strip())
            ...
            # match paragraph numbers with additional text and split
                ...
                line = line.split(" ", 1)
                if len(line) > 1 and lines[i-1][-1] in '.,:;"' and not line[1].startswith("und") and not line[0].endswith(","):
                    if para:
                        clean_lines.append(para.strip())
                        para = ""
