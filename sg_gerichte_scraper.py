...
# python3.10 sg_gerichte_scraper.py -p SG_Gerichte -s SG_Gerichte_clean
...
def get_pages(lines: List[str]) -> str | None:
    page_pattern = r"(©\sKanton\sSt.Gallen\s\d{4}\s)?Seite\s\d{1,3}\/\d{1,3}"
    if re.findall(page_pattern, " ".join(lines)):
        ...
        pages = [page.split("/")[0] for page in pages]
    else: return None
    if pages:
...
def get_paras(lines: List[str]) -> List[str]:
    ...
    for i, line in enumerate(lines):
        ...
        if line == "Publikationsplattform" or line == "St.Galler Gerichte":
            headnote_counter += 1
        ...
        if sachverhalt_counter == 0 and line:
            ...
            elif (re.match(absatz_pattern, line) or re.match(absatz_pattern2, line) or re.match(absatz_pattern3, line)) and not re.match(datum_pattern, line) and not line.endswith("Kammer") and not re.match("^\w\._+", line) and ((lines[i-1] or lines[i-2] or lines[i-3]) and (lines[i-1] or lines[i-2] or lines[i-3])[-1] != ",") and not line.startswith("V. R. W."):
