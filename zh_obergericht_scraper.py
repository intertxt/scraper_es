...
def split_lines(parsed_text: str) -> List[str]:
    rem_spaces = re.sub(r"\s+", r"\s", parsed_text)
    unesc = unescape(rem_spaces)
    #rem_hex = BeautifulStoneSoup(rem_spaces,
     #              convertEntities=BeautifulStoneSoup.ALL_ENTITIES).contents[0]
    #print(rem_hex)
    split_lines = [line.strip().replace("     ", " ").replace("\uf02d", "").replace(" ", "").replace(" ", "") for line in unesc.split("\n")]
    return split_lines
...
def get_paras(lines: List[str]) -> List[str]:
    ...
    for i, line in enumerate(lines):
        ...
        if line and line[0].isdigit() and line[-1].isdigit() and lines[i - 1]:
            if para:
                # joining initial char that was split from word
                # if para_counter <= 2:
                if re.findall(r"\s[a-zA-HJ-Z]\s\w+", para):
                    mlist = re.findall(r"\s[a-zA-HJ-Z]\s\w+", para)
                    for m in mlist:
                        nospace = re.sub(" ", "", m)
                        new = " " + nospace
                        para = re.sub(m, new, para)
                # end replace split initial char
        ...
        # match paragraph numbers with additional text and split
            ...
            # if para_counter <= 2:
                for l in line:
                    mlist = re.findall(r"\s[a-zA-HJ-Z]\s\w+", l)
                    for m in mlist:
                        nospace = re.sub(" ", "", m)
                        new = " " + nospace
                        new_l = re.sub(m, new, l)
        ...
        # match roman numbered subtitles
        elif (re.fullmatch(r"[IVX]+\.", line) or re.fullmatch(r"[IVX]+\.\s.+", line)) and line[i+1]:
            if para:
                # joining initial char that was split from word
                # if para_counter <= 2:
                if re.findall(r"\s[a-zA-HJ-Z]\s\w+", para):
...
def get_paras_hg(lines: List[str]) -> List[str]:
    ...
    for i, line in enumerate(lines):
        ...
        # get start of main text
        if line:
            if sachverhalt_counter == 0 and ("Erwägung" in line or "Rechtsbegehren" in line or "erwägt" in line or "Sachverhalt" in line or "in Sachen" in line) and len(lines) > 50:
                clean_lines.append(line.strip())
                sachverhalt_counter += 1
                continue
            ...
            if sachverhalt_counter == 0 and line and len(lines) > 50:
                ...
                # match roman numbered subtitles
                elif (re.fullmatch(r"[IVX]+\.", line) or re.fullmatch(r"[IVX]+\.\s.+", line)):
                    if para:
                        # joining initial char that was split from word
                        # if para_counter <= 2:
                        if re.findall(r"\s[a-zA-HJ-Z]\s\w+", para):
...
def main():
    ...
    file_list = ["ZH_OG_001_PQ140027_2014-07-08", "ZH_BK_004_ED200048-L-Z1_2020-10-20"]
    for filename in sorted(os.listdir(PATH_TO_DATA))[7058:]:
