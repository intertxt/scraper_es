...
# python3.10 vs_gerichte_scraper.py -p VS_Gerichte -s VS_Gerichte_clean
...
def get_pages(lines: List[str], filename) -> str | None:
    # KRK-: Seite; CCR-: integer; RVJ- & ZWR-: integer (mit Kopfzeile);
    if "KRK-" in filename:
        page_pattern = r"Steuerrekurskommission.+\s+Seite\s\d{1,3}"
        pages = [lines.pop(lines.index(line)).rsplit(" ", 1)[-1] for line in lines if re.fullmatch(page_pattern, line)]
    elif "CCR-" in filename or "RVJ-" in filename or "ZWR-" in filename:
        pages = [lines.pop(lines.index(line)).strip() for line in lines if line.strip().isdigit()]
        if not pages or pages[0] == "0" or pages[-1] == "0":
            pages = filename.split("S--")[-1].split("_")[0] if "S--" in filename else filename.split("p--")[-1].split("_")[0]
            if re.fullmatch("\d{1,3}\-\d{1,3}", pages):
                for i in range(int(pages.split("-")[0]), int(pages.split("-")[0])):
                    if str(i) in line and (line.strip().isdigit() or "RVJ" in line or "ZWR" in line):
                        lines.pop(lines.index(line))
                return pages
            else: return ""
    else:
        pages = [lines.pop(lines.index(line)).strip("- ") for line in lines if line.startswith("-") and line.endswith("-") and line.strip("- ").isdigit()]
    if pages:
...
def get_paras(lines: List[str]) -> List[str]:
    ...
    for i, line in enumerate(lines):
        ...
        if sachverhalt_counter == 0 and line:
            ...
            elif (re.match(absatz_pattern, line) or re.match(absatz_pattern2, line) or re.match(absatz_pattern3, line)) and not re.match(datum_pattern, line) and not re.match("[A-Z]\.?_+", line):
...
def build_xml_tree(filename: str, loaded_json, filter_list: List, footnotes=None, pages=None, date=None):
    """Build an XML-tree."""
    ...
    if "nodate" in filename:
        text_node.attrib["year"] = date.split("-")[0]
    else:
        if "-" in loaded_json["Datum"]:
            ...
            text_node.attrib["year"] = loaded_json["Datum"][-4:]
    if "nodate" in filename:
        text_node.attrib["decade"] = date.split("-")[0][:-1]+"0"
    else:
        if "-" in loaded_json["Datum"]:
...
def main():
    different_file_types = ['TDMA', 'TDEN', 'KRK-', 'BGBR', 'TRA-', 'CCR-', 'TCVS', 'RVJ-',
                            'ZWR-', 'BGVI', 'TDHC', 'TDMO', 'Micr', 'TDSI', 'KGVS', 'BGLE']
    for filename in sorted(os.listdir(PATH_TO_DATA)):
        if filename.endswith("pdf") and not "Micro" in filename and not filename.startswith("VS_BZG_999_CCR-20200903-FI-19-1_nodate"): #or filename.startswith("SZ_KG_002_ZK2-2020-84_2021-03-03") or filename.startswith("SZ_KG_003_STK-2013-22_2013-09-03") or filename.startswith("SZ_KG_004_BEK-2014-210_2015-01-29") or filename.startswith("SZ_KG_999_GPR-2017-12_2017-12-13")):
            ...
            pages = get_pages(lines, filename)
            ...
            date = ""
            if "nodate" in filename:
                # TODO extract lang and based on lang the date
                if detect(" ".join(clean_text)) == "fr":
                    date_pattern = r"du\s\d{1,2}\s(janvier|février|mars|avril|mai|juin|juillet|aout|septembre|octobre|novembre|décembre)\s\d{2,4}"
                    month_to_num = {"janvier":"01",
                                   "février":"02",
                                    "mars":"03",
                                    "avril":"04",
                                    "mai":"05",
                    ...
                    date_pattern = r"vom\s\d{1,2}\.\s(januar|februar|märz|april|mai|juni|juli|august|september|oktober|november|dezember)\s\d{2,4}"
                    month_to_num = {"januar": "01",
                                    "februar": "02",
                                    "märz": "03",
                                    "april": "04",
                                    "mai": "05",
                                    ...
                                    "november": "11",
                                    "dezember": "12"}
                for para in clean_text[:20]:
                    para = para.lower()
                    if re.search(date_pattern, para.lower()):
                        match = re.search(date_pattern, para.lower()).group(0)
                        date = match.split()[-1] + "-" + month_to_num[match.split()[2]] + "-" + match.split()[1].strip(".")
                        break
                filename = filename.replace("nodate", date) #yyyy-mm-dd
            ...
            if json_name in sorted(os.listdir(PATH_TO_DATA)):
                with open(os.path.join(PATH_TO_DATA, json_name), "r", encoding="utf-8") as json_file:
                    ...
                    if date:
                        tree = build_xml_tree(filename, loaded_json, clean_text, date=date)  # generates XML tree
                    else:
                        tree = build_xml_tree(filename, loaded_json, clean_text)
