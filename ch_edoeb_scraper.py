...
# python3.10 ch_edoeb_scraper.py -p CH_EDOEB -s CH_EDOEB_clean
...
def get_pages(lines: List[str], filename: str) -> str | None:
    year = "nodate" if "nodate" in filename else int(re.search(r"\d{4}", filename).group(0))
    if type(year) == int and year >= 2006 or year == "nodate":
        ...
        page_pattern = r"(page\s?)?\d{1,3}"
        pages = [lines.pop(lines.index(line)).split()[1] if line.startswith("p") else line.strip() for line in lines if re.fullmatch(page_pattern, line)]
    if pages:
...
def main():
    for filename in sorted(os.listdir(PATH_TO_DATA)):
        if filename.endswith("pdf") and not filename.startswith("CH_ED%C3%96B_001_unbekannt_2018-04-20"):
            ...
            if "spam" in parsed_text or ".doc" in parsed_text or parsed_text.count("Ω")>3:
                continue
