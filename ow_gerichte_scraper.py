...
parser.add_argument("-d","--directory",type=str,default="OW_Gerichte",help="current directory for the scraper")
...
SAVE_PATH = "/home/admin1/tb_tool/clean_scraper_data/OW_Gerichte_clean/"
...
def parse_text(parsed_html) -> List[str]:
	...
	text = []
	tag_list = parsed_html.findAll(["title", "table", "p" ])
	for i, tag in enumerate(tag_list):
		if tag.name == "table":
			...
			tag_text = unicodedata.normalize('NFKD', tag.get_text(separator="<br>", strip=True)).replace("\n", " ").replace("<br>", " ")
			if tag_text == "":
				continue
			else:
