...
parser.add_argument("-d","--directory",type=str,default="LU_Gerichte",help="current directory for the scraper")
...
SAVE_PATH = "/home/admin1/tb_tool/clean_scraper_data/LU_Gerichte_clean/"
...
def parse_text(parsed_html) -> List[str]:
	...
	text = []
	tag_list = parsed_html.findAll(["h1", "th", "td" ])
	for i, tag in enumerate(tag_list):
		# this code leaves all the <br> tags in the code so that the text can be split on them afterwards
		# it also already strips the text snippet of whitespace characters
		text += unicodedata.normalize('NFKD', tag.get_text(separator="<br>", strip=True)).split("<br>")
	return text
	...
	for i, elem in enumerate(text_wo_hyphens):
		...
		# else:
			if re.match(datum_pattern, elem):
				...
				if elem.startswith(match + "______"):
					paragraph_list.append(elem)
					# if para != "" and para[-1] != " ":
					# 	para += " " + elem
					# else:
