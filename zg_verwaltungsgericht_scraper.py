...
# python3.10 zg_verwaltungsgericht_scraper.py -p ZG_Verwaltungsgericht -s ZG_Verwaltungsgericht_clean
...
def get_paras(lines: List[str]) -> List[str]:
    ...
    headnote_counter = 0
    headnote_pattern = r"^Urteil\s[A-Z]\s\d{4}\s\d{1,4}$"
    ...
    for i, line in enumerate(lines):
        ...
        if re.fullmatch(r"^[A-Z]\s\d{4}\s\d{1,4}$", line):
            clean_lines.append(line.strip())
