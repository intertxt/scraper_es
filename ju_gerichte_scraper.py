...
# python3.10 ju_gerichte_scraper.py -p JU_Gerichte -s JU_Gerichte_clean
...
absatz_pattern3 = r"^(([A-D]\.)+((\s)?[a-z]\)|\d(\.)?)*|\d{1,3}((\.\s)|([a-z]{1,3}\)\.?)|(\s[a-z]\)))|§.*:|[a-z]{1,2}\)(\s[a-z]{1,2}\))?|\d{1,3}\.)"
...
def get_pages(lines: List[str]) -> str:
    pages = [lines.pop(lines.index(line)).strip() for line in lines if line.isdigit()]
    if pages:
...
def get_paras(lines: List[str]) -> List[str]:
    ...
    headnote_counter = 0
    footnote_pattern = r"^TPI\/\d+\/\d+\s–\s.+$"

    for i, line in enumerate(lines):
        line = line.strip()
        if re.fullmatch(footnote_pattern, line):
            headnote_counter += 1
            if headnote_counter > 1:
                continue

        # get start of main text 
        if (" fait" in line.lower() or "en fait :" in line.lower() or "attendu" in line.lower()) and sachverhalt_counter == 0:
            clean_lines.append(line.strip())
            sachverhalt_counter += 1
            continue

        elif line.lower().startswith("vu") and sachverhalt_counter == 0:
            sachverhalt_counter += 1
            para += line
            continue
            
        elif line and sachverhalt_counter == 0:
            clean_lines.append(line.strip())
            ...
            elif (re.fullmatch(absatz_pattern, line) or re.fullmatch(absatz_pattern2, line) or re.fullmatch(absatz_pattern3, line)) and not re.fullmatch(datum_pattern, line) and not lines[i-1].endswith("CHF") and not re.match("\w\._+", line):
                if para:
                    clean_lines.append(para.strip())
                    para = ""
                clean_lines.append(line)
            ...
            elif (re.match(absatz_pattern, line) or re.match(absatz_pattern2, line) or re.match(absatz_pattern3, line)) and not re.match(datum_pattern, line) and not line.endswith("Kammer") and not lines[i-1].endswith("CHF") and not re.match("\w\._+", line):
                line = line.split(" ", 1)
                if para:
                    clean_lines.append(para.strip())
                    para = ""
...
def build_xml_tree(filename: str, loaded_json, filter_list: List, footnotes=None, pages=None):
    ...
    if "0000" in filename:
        text_node.attrib["date"] = get_date(filter_list)
    else:
    ...
    if "0000" in filename:
        text_node.attrib["year"] = get_date(filter_list)[:4]
    else:
        if "-" in loaded_json["Datum"]:
            ...
            text_node.attrib["year"] = loaded_json["Datum"][-4:]
    if "0000" in filename:
        text_node.attrib["decade"] = get_date(filter_list)[:3] + "0"
    else:
        if "-" in loaded_json["Datum"]:
...
def get_date(lines) -> str:
    """
    Extracts date out of text where date is missing in filename.
    ...
    for line in lines:
        if line.startswith("JUGEMENT DU") or line.startswith("ORDONNANCE DU") or line.startswith("DECISION DU") or line.startswith("ARRET DU") or line.startswith("ARRÊT DU") or line.startswith("DÉCISION DU") :
            line = line.split()
            day = line[-3] if len(line[-3]) == 2 else day_conversions[line[-3]]
            month = month_conversions[line[-2]]
            year = line[-1]
...
def main():
    for filename in sorted(os.listdir(PATH_TO_DATA)):
        if filename.endswith("pdf"):
            ...
            if filename.endswith("nodate.pdf"):
                xml_filename = filename.replace("nodate.pdf", "0000-00-00.xml")
            elif "0000-00-00" in filename:
                date = get_date(lines)
                xml_filename = filename.replace("0000-00-00.pdf", date+".xml")
            else:
