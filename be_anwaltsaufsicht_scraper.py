...
# python3.10 be_anwaltsaufsicht_scraper.py -p BE_Anwaltsaufsicht -s BE_Anwaltsaufsicht_clean
...
def split_lines(parsed_text: str) -> List[str]:
    split_lines = [line.strip().replace("     ", " ").replace("\uf02d", "").replace("            ", "").replace("……………...", "").replace("……… ", "").replace("   ", " ").replace("  ", " ") for line in parsed_text.split("\n")]
    return split_lines
...
def build_xml_tree(filename: str, loaded_json, filter_list: List, pages: str): # removed footnotes argument
    """Build an XML-tree."""
...
def main():
    for filename in sorted(os.listdir(PATH_TO_DATA)):
        if filename.endswith("pdf"):
            ...
            lines_wo_hyphens = remove_hyphens(lines) # stopped here 
