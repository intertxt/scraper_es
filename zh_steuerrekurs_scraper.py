...
# python3.10 zh_steuerrekurs_scraper.py -p ZH_Steuerrekurs -s ZH_Steuerrekurs_clean
...
absatz_pattern = r"\d{1,3}\.\s\w\)|(\s)?[0-9]+\.([0-9]+(\.)?)*(\s-\s[0-9]+\.([0-9]+(\.)?)*)?"
...
def split_lines(parsed_text: str) -> List[str]:
    split_lines = [line.strip().replace("     ", " ").replace("    ", " ").replace("   ", " ").replace("  ", " ").replace("\uf02d", "").replace(" ", "").replace(" ", "").replace("\n", "") for line in parsed_text.split("\n\n")]
    return split_lines
    ...
    for i, line in enumerate(lines):
        ...
        if line:
            if re.fullmatch(code_pattern, line):
                code_counter += 1
                if code_counter > 1: continue

            if "Erwägungen:" in line or "hat sich ergeben:" in line:
            ...
            if sachverhalt_counter == 0 and line:
                ...
                # match paragraph numbers with additional text and split
                    ...
                    para_mark = re.match(absatz_pattern, line) or re.match(absatz_pattern2, line) or re.match(absatz_pattern3,
                                                                                                    line)
                    para_mark = para_mark.group(0)
                    if para:
                        clean_lines.append(para.strip())
                        para = ""
                    clean_lines.append(para_mark)
                    para += line[len(para_mark):] if re.match(r".*\w+-$", line) else line[len(para_mark):] + " "
                    pm_counter += 1
                    # line = line.split(" ", 1)
                    # if para:
                    #     clean_lines.append(para.strip())
