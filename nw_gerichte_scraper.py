...
# python3.10 nw_gerichte_scraper.py -p NW_Gerichte -s NW_Gerichte_clean
...
absatz_pattern = r"^[0-9]{1,2}\.([0-9]{1,2}(\.)?)*"
...
def split_lines(parsed_text: str) -> List[str]:
    split_lines = [line.strip().replace("     ", " ").replace("\uf02d", "").replace(" ", "").replace("&amp; ", "").replace("________________________________________________________________________________________", "") for line in parsed_text.split("\n")]
    return split_lines


def get_pages(lines: List[str]) -> str | None:
    page_pattern = r"\d{1,3}\s\│\s\d{1,3}"
...
def get_paras(lines: List[str]) -> List[str]:
    ...
    for i, line in enumerate(lines):
        ...
        if  "in Anbetracht dessen," in line or "Sachverhalt" in line or "S a c h v e r h a l t" in line  or "Sachverhalt:" in line :
        ...
        if sachverhalt_counter == 0 and line:
            ...
            if (re.fullmatch(absatz_pattern, line) or re.fullmatch(absatz_pattern2, line) or re.fullmatch(absatz_pattern3, line)) and not re.fullmatch(datum_pattern, line) and not re.match("^\w\._+", line):
                if para:
                    clean_lines.append(para.strip())
                    para = ""
                clean_lines.append(line)
            ...
            elif (re.match(absatz_pattern, line) or re.match(absatz_pattern2, line) or re.match(absatz_pattern3, line)) and not re.match(datum_pattern, line) and not line.endswith("Kammer") and not re.match("^\w\._+", line) and ((lines[i-1] or lines[i-2] or lines[i-3]) and (lines[i-1] or lines[i-2] or lines[i-3])[-1] != ","):
