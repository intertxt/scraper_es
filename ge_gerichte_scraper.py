...
parser.add_argument("-d","--directory",type=str,default="GE_Gerichte",help="current directory for the scraper")
...
SAVE_PATH = "/home/admin1/tb_tool/clean_scraper_data/GE_Gerichte_clean/"
...
def parse_text(parsed_html) -> List[str]:
	...
	text = []
	tag_list = parsed_html.findAll(["span", "p", "table"])
	for i, tag in enumerate(tag_list):
		if tag.name == "table":
			text.append(str(tag))
		else:
			if tag_list[i-1].name == "table":
				continue
			else:
				tag_text = unicodedata.normalize('NFKD', tag.get_text()).strip().replace("        ", " ")
				tag_text = tag_text.replace("   ", "")
				tag_text = re.sub("!.*\s", "", tag_text)
				text.append(tag_text.replace("      ", " "))
