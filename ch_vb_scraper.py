...
# python3.10 ch_vb_scraper.py -p CH_VB -s CH_VB_clean
...
absatz_pattern3 = r"^((\s)?[IVMDC]\.(\/)?((\s)?[a-z])*|[IVD]{1,4}\.)\s"
...
def get_pages(lines: List[str]) -> str:
    page_pattern = r"Seite\s\d{1,4}\-\d{1,4}"
    pages = re.findall(page_pattern, " ".join(lines))
    if pages:
        pages = pages[0].strip("Seite ")
        return pages
    pages = [lines.pop(lines.index(line)) for line in lines if line.strip().isdigit() and len(line.strip()) <= 4]
    if pages:
