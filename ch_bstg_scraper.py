...
# python3.10 ch_bstg_scraper.py -p CH_BSTG -s CH_BSTG_clean
...
def get_pages(lines: List[str]) -> str:
    page_pattern = r"-\s\d{1,3}\s-"
    pages = [lines.pop(lines.index(line)).strip("- ") for line in lines if re.fullmatch(page_pattern, line.strip())]
    if pages:
...
def main():
    ...
    for filename in sorted(os.listdir(PATH_TO_DATA)):
        if filename.endswith("pdf"): # and ("CH_BSTG_001_BA-2010-3_2010-09-16" in filename or "CH_BSTG_001_AU-2007-1_2007-10-24" in filename or "CH_BSTG_001_BB-2005-1_2005-02-15" in filename):
