...
# can be run with the following CLI prompt:
# python3.6 ch_bge_scraper.py -p /home/admin1/tb_tool/scraping_data/
...
absatz_pattern = r"^(\s)?[0-9]+(\.)?(\s)?([a-z]\)|([0-9]+(\.)?)*(\s-\s[0-9]+\.([0-9]+(\.)?)*)?)(\s?[a-i]\))?"
absatz_pattern2 = r"^([a-i]\))?((\s)?[0-9]+\.([0-9]+(\.)?)*(\s-\s[0-9]+\.([0-9]+(\.)?)*)?\s-\s[0-9]+\.([0-9]+(\.)?)*(\s-\s[0-9]+\.([0-9]+(\.)?)*)?)?"
absatz_pattern3 = r"([A-Z]([IVCMD]+)?(\.|\))-?(\s[a-z]\))?|(\s*[a-z]{1,3}(\)|\.))+|\d{1,3}((\.\s)|(\s[a-z]\)))|§.*:|[a-z]{1,2}\)(\s[a-z]{1,2}\))?|\d{1,3}\.)"
datum_pattern = r"[0-9][0-9]?\.([\s]{1,2}([A-Z][a-z]+|März)|[0-9]{1,2}\.)\s?[1-9][0-9]{3}"
			...
			if elem.startswith("<table"):
				...
				if re.match(datum_pattern, elem):
					...
					if elem.startswith(match + "__") or elem.startswith(match + "Abteilung") or elem.startswith(
							match + "Kammer") or elem.startswith(match + "Auszug") or (elem[len(match):].strip() and elem[len(match):].strip()[0].isupper()):
						paragraph_list.append(elem.strip())
					else:
						paragraph_list.append(match.strip())
						paragraph_list.append(elem[len(match):].strip())
				elif re.match(absatz_pattern2, elem):
					match = re.match(absatz_pattern2, elem).group(0)
					if elem.startswith(match + "__") or elem.startswith(match + "Abteilung") or elem.startswith(
						...
						paragraph_list.append(match.strip())
						paragraph_list.append(elem[len(match):].strip())
				elif re.match(absatz_pattern3, elem) and elem.split()[1].strip()[0].isupper():
...
def iterate_files(directory, filetype):
	...
	# with open("ch_bge_json_xmls.txt") as f:
	# 	jsnames = f.read().split()
	# 	weird_jsons = [name.strip(".xml\n") for name in jsnames]
	...
	for filename in sorted(os.listdir(directory)):
		if filename.endswith(".html"): # and filename.strip(".htmljson") in weird_jsons: #and "nodate" in filename and filename.strip("nodate.html") in files:
